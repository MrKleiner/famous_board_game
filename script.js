window.allcards = [
	...
	// біографія
	{
		...
		'photo': 'kopernik',
		'date': '(February 19, 1473 - May 24, 1543)',
		'cid': 'qcn'
	},
	{
		'name': 'Adam Malysz',
	...
	},
	{
		...
		'photo': 'andrei_wajda',
		'date': '(March 6, 1926 - October 9, 2016)',
		'cid': 'My0'
	},
	{
		'name': 'Czesław Miłosz',
		...
		'photo': 'milosh',
		'date': '(June 30, 1911 - August 14, 2004)',
		'cid': 'qQX'
	},
	{
		'name': 'Zbigniew Preisner',
		'desc': 'Zbigniew Preisner is best known as the author of music for the films of the famous Polish director Krzysztof Kieslowski. In some of them Praisner\'s music is assigned to the fictional Dutch composer Van den Budenmaer.',
		'photo': 'preisner',
		'date': '(May 20, 1955)',
		'cid': 'qYT'
	},
	{
		'name': 'Robert Witold Makłowicz',
	...
	},
	{
		...
		'photo': 'marek',
		'date': '(February 22, 1934 - October 3, 1992)',
		'cid': 'FYN'
	},
	{
		...
		'photo': 'polish_giger',
		'date': '(February 24, 1929 - February 21, 2005)',
		'cid': 'xYO'
	}
]
...
document.addEventListener('change', event => {
    console.log('click_registered');
    // pool
    const pool = event.target.closest('#game_players input');
    if (pool) {
    	if (parseInt($(pool).attr('max')) < $(pool).val()){
    		$(pool).val($(pool).attr('max'))
    	}

       	if (parseInt($(pool).attr('min')) > $(pool).val()){
    		$(pool).val($(pool).attr('min'))
    	}
    }
});
...
window.mrk_ect_tooltip_timeout = 50; // tooltip timeout
...
document.addEventListener('mouseover', event => {
	...
	if (cursor_over_tooltip_obj)
	{
		mrk_ect_timer = setTimeout(function() {
			...
			var etgt = cursor_over_tooltip_obj;

			var target = cursor_over_tooltip_obj

			var celemheight = $(target).height()
			var celemwidth = $(target).width()

		    // Get the bounding rectangle of target
		    var rect = target.getBoundingClientRect();

		    // Mouse position
		    var x = etgt.clientX - rect.left;
		    var y = etgt.clientY - rect.top;

		    var createshift = rect.bottom

			// $('.mrk_ect_tooltip_box').css({top: window.cs_y + 'px', left: (window.cs_x - 30) + 'px'});
			$('.mrk_ect_tooltip_box').css({top: createshift + 'px', left: (window.cs_x - 30) + 'px'});
			...
			$('#game_table').append(
				`
					<div class="table_card">
						...
						<div class="table_card_date">` + rnd_item['date'] + `</div>
		...
		$('#game_table').append(
			`
				<div class="table_card">
					...
					<div class="table_card_date">` + cr['date'] + `</div>
